...
**Public artifact version 0.1.0, 2026-07-30**
...
written into conversational history, recovery begins from a corrupted action
trace. Existing work evaluates function-call correctness, structured-output
validity, and final environment state, but does not isolate admission failure
atomicity at the boundary between a generated action batch and its executor.
...
positions produced the same contrast. A prevalence probe of six pinned Python
framework surfaces found partial admission in five; the sixth rejected raw
malformed arguments at its typed core boundary, while its provider-adapter
behavior remains unresolved. These results establish the boundary invariant
and its practical relevance, not end-to-end task improvement.
...
5. It measures the admission behavior of six pinned framework surfaces under
   the same valid-then-malformed batch.
...
### 4.3 Framework prevalence probe

The prevalence artifact installs pinned releases in an isolated environment
...
- whether malformed action state entered framework history;
...
tool execution path. They do not establish behavior for every provider,
configuration, release, or language implementation.
...
- framework-surface partial-admission prevalence.
...
The prevalence artifact is `artifact/results/framework_prevalence.json`.
Five of six pinned framework surfaces exhibited partial admission:
...
committed and malformed response state remained represented in history or the
framework's run state.

LlamaIndex core 0.14.23 rejected the malformed raw argument before constructing
a `ToolCall` event. As noted above, this leaves provider-adapter batch behavior
unresolved. The measured prevalence is therefore five vulnerable surfaces out
of six tested surfaces, not an estimate over all framework deployments.

These results demonstrate the admission invariant in the isolated harness and
show that the counterexample is not unique to the motivating runtime. They do
...
![Framework prevalence matrix](../figures/framework_prevalence.svg)

**Figure 4.** Five tested framework surfaces partially admitted a mixed valid and malformed batch. The tested LlamaIndex core boundary structurally rejected raw malformed arguments; its provider adapters remain unresolved.
...
boundary harness, exhaustive truncation of one representative payload, and one
probe each of six pinned Python framework surfaces. Five surfaces admitted a
valid call before handling a malformed peer, but this convenience sample is not
a population-frequency estimate. It does not cover every framework release,
configuration, provider adapter, language implementation, or custom execution
loop. The CrewAI result uses a released but deprecated executor path. The
LlamaIndex result is limited to its typed core boundary and leaves provider
...
Malformed tool output should not be handled as ordinary conversational text or
as the first failed command in a sequential batch. It is an admission failure.
The appropriate boundary validates the complete batch, preserves completed
content separately, records what was actually admitted and executed, and
derives recovery from that record.
